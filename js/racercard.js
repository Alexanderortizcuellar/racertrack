/**
 * Generates a single Racer card HTML block.
 * @param {Object} racer - Racer data object.
 * @param {number} racer.id
 * @param {number} racer.rank
 * @param {number} racer.score
 * @param {number} racer.players
 * @param {number} racer.puzzles_done
 * @param {Array} racer.puzzles - Array of puzzle objects { lichess_id, rating, solved, time }.
 * @returns {string} HTML for a single racer card.
 */
export function createRacerCard(racer) {
  ...
  racer.puzzles.forEach((p) => {
    html += `
      <tr>
        ...
        <td>${p.rating}</td>
        <td><span  style="color:white" class="badge ${p.solved ? "badge-solved" : "badge-unsolved"}" >
        ${p.solved ? "✔" : "✘"}
        </span></td>
        <td>${p.time}s</td>
      </tr>
